...
class FetchCredentialStatusImplTest {
    ...
    private lateinit var useCase: FetchCredentialStatus
    ...
    fun `Fetching credential status for token status list returns credential status`() = runTest {
        val result = useCase(ISSUER, mockTokenStatusListProperties)
    ...
    private fun success() {
        coEvery {
            mockFetchStatusFromTokenStatusList(ISSUER, mockTokenStatusListProperties)
        } returns Ok(credentialStatus)
    }

    private companion object {
        val credentialStatus = CredentialStatus.VALID
        const val ISSUER = "credentialIssuer"
    }
}
